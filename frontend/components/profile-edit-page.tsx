...
function AvatarUploader({
  ...
  return (
    <div className="relative mx-auto w-fit">
      <div className={`${dim} overflow-hidden rounded-full border-4 border-white shadow-lg bg-gradient-to-br from-slate-100 to-slate-200`}>
        {avatarUrl ? (
...
function DisplayField({ label, value, icon }: { label: string; value: string; icon: React.ReactNode }) {
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex h-10 items-center rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm text-slate-700">
        {value || <span className="text-slate-300 italic">ไม่ระบุ</span>}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-1 flex-col rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/60">
      ...
      <div className="rounded-t-3xl bg-gradient-to-r from-slate-800 to-slate-700 px-6 pb-8 pt-6">
        ...
        <AvatarUploader avatarUrl={data.avatarUrl} editable={false} size="lg" />
        ...
        <div className="mt-4 text-center">
          <p className="text-lg font-bold text-white">
            {data.firstName || data.surname
              ...
              : <span className="text-slate-500 italic text-sm">ยังไม่ได้ตั้งชื่อ</span>}
          </p>
          {data.email && <p className="mt-0.5 text-xs text-slate-400">{data.email}</p>}
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col gap-4 p-6">
        <div className="grid grid-cols-2 gap-4">
          <DisplayField label="ชื่อ" value={data.firstName} icon={<User className="h-3 w-3" />} />
          <DisplayField label="นามสกุล" value={data.surname} icon={<User className="h-3 w-3" />} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <DisplayField label="โทรศัพท์" value={data.phone} icon={<Phone className="h-3 w-3" />} />
          <DisplayField label="อีเมล" value={data.email} icon={<Mail className="h-3 w-3" />} />
        </div>
    ...
    <div className="flex flex-1 flex-col rounded-3xl border border-[#E8614A]/30 bg-white shadow-xl shadow-orange-100/60">
      ...
      <div className="rounded-t-3xl bg-gradient-to-r from-[#E8614A] to-[#f0825f] px-6 pb-8 pt-6">
        ...
        <AvatarUploader
          ...
          editable={true}
          onFileChange={(url) => onChange("avatarUrl", url)}
          size="lg"
        />
        <p className="mt-3 text-center text-xs text-orange-100/80">
          กดปุ่ม <strong className="text-white">+</strong> เพื่อเปลี่ยนรูปโปรไฟล์
        </p>
      </div>
...
export default function ProfileEditPage({
  ...
  const handleConfirm = async () => {
    try {
      ...
      console.error(err)
      showToast("บันทึกไม่สำเร็จ", "info")
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-orange-50/30 to-white font-sans">
      ...
      <main className="mx-auto max-w-5xl px-6 py-12">
        ...
        <div className="mb-10 text-center">
          <h1 className="text-3xl font-bold text-slate-800">ข้อมูลโปรไฟล์</h1>
          <p className="mt-2 text-sm text-slate-500">จัดการข้อมูลส่วนตัวและรูปโปรไฟล์ของคุณ</p>
        </div>

        {/* Two-panel layout */}
        <div className="flex flex-col gap-6 md:flex-row md:items-stretch">
          {/* LEFT — View (always visible, takes full width when not editing) */}
          <div className={`flex ${isEditing ? "flex-1" : "mx-auto w-full max-w-sm"}`}>
            <ViewCard data={saved} onEdit={handleEdit} />
          </div>

          {/* Divider + RIGHT — only visible when editing */}
          {isEditing && (
            <>
              {/* Divider */}
                ...
                <div className="flex h-full flex-col items-center gap-2">
                  ...
                  <div className="rounded-full border border-slate-200 bg-white p-2 shadow-sm">
                    <svg viewBox="0 0 16 16" className="h-4 w-4 text-slate-400" fill="none" stroke="currentColor" strokeWidth="1.5">
                      <path d="M2 8h12M9 5l3 3-3 3" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </div>
                  <div className="flex-1 w-px bg-gradient-to-b from-transparent via-slate-200 to-transparent" />
                </div>
