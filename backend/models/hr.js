...
export const GetMemberModel = async(owner_id, post_id, setoff, limit, filter) => {
    let query = `
        SELECT u.id AS user_id, u.firstname AS user_firstname, u.lastname AS user_lastname, r.ai_score, r.faculty_match, m.status
        FROM posts p 
...
export const GetMemberResumeResultModel = async (member_id, owner_id, post_id) => {
    const [result] = await db.query(`
        SELECT r.resume_url, r.transcript_url, r.ai_score, r.storytelling_score, r.overall_confidence, r.skills, 
        m.status, r.faculty_match, r.specific_strengths, recommendation_reason
        FROM posts p
        JOIN members m ON m.post_id = p.id
